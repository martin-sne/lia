...
def UpdateSNMPObjs():
	...
	for child_of_root in root:
        	...
        	for table in tree.iterfind(path):
                	...
                	# build path for xpath query 
                	# Example: 'Zone[@name="os3.nl"]/table[@name="dnssecGlobalTable"]/item/data' 
				...
				LogMsg("Beginning data update.")
                        	...
                        	# dnssecZoneAuthNsTableRow2.setRowCell(2, agent.DisplayString("ns1.distributed-systems.net"))
                        	tables['name'] + "Row" + zones['id'] + '.setRowCell(' + data.attrib['id'] + ', agent.' + data.attrib['type'] + '(' + data.text + '))'
				print tables['name'] + "Row" + zones['id'] + '.setRowCell(' + data.attrib['id'] + ', agent.' + data.attrib['type'] + '(' + data.text + '))'
				LogMsg("Loading dnssecZoneCount")

				msg = "Updating \"" + data.attrib['name'] + "\" object with data \"" + data.text + "\"."
				LogMsg(msg.format(dataformat))
